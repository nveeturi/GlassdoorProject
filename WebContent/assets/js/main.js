window.Mapnificent = (function(window, document, $, undefined) {
	...
	var func = function(useroptions) {
		...
		that.addToMap = function(mapObject) {
			...
			that.canvasoverlay = new CanvasOverlay(that.env.mapGStartCenter,
					that.canvas_id, onaddcallback, that.map);
			google.maps.event.addListener(that.map, "zoom_changed",
					function(oldLevel, newLevel) {
						that.ctx.clearRect(0, 0, that.canvas.width,
								that.canvas.height);
		...
		that.moveMapPosition = function() {
			...
			that.mapBoundsXY = that.canvasoverlay
					.fromLatLngToDivPixel(that.mapBounds.getSouthWest());
			that.canvasoverlayxy = that.canvasoverlay
					.fromLatLngToDivPixel(that.canvasoverlay.getPoint());
			var boundnexy = that.canvasoverlay
					.fromLatLngToDivPixel(that.mapBounds.getNorthEast());
			var need = false;
			if ((that.mapBoundsXY.x - that.widthCacheOffset * (1 / 3)) < that.canvasoverlayxy.x) {
				need = true;
			} else {
				if ((boundnexy.x + that.widthCacheOffset * (1 / 3)) > that.canvasoverlayxy.x
						+ that.canvas.width) {
					need = true;
				} else {
					if ((that.mapBoundsXY.y + that.heightCacheOffset * (1 / 3)) > that.canvasoverlayxy.y) {
						...
						if ((boundnexy.y - that.heightCacheOffset * (1 / 3)) < that.canvasoverlayxy.y
								- that.canvas.height) {
							need = true;
						}
					}
		...
		that.getCanvasPosition = function() {
			var pxnpm = new google.maps.Point(that.mapBoundsXY.x
					- that.widthCacheOffset, that.mapBoundsXY.y
					+ that.heightCacheOffset);
		...
		that.setCanvasDimensions = function() {
			...
			that.canvas.height = that.mapSize.height
					* that.env.heightCacheFactor;
		};
		...
		that.redraw = function() {
			...
			if (layerCount > 0) {
				...
				layerArr.sort(function(a, b) {
					return a.layerObject.getDrawingLevel()
							- b.layerObject.getDrawingLevel();
				});
		...
		that.getDistanceInKm = function(pos1, pos2) {
			var R = 6371, DegToRadFactor = Math.PI / 180;
			return Math.acos(Math.sin(pos1.lat * DegToRadFactor)
					* Math.sin(pos2.lat * DegToRadFactor)
					+ Math.cos(pos1.lat * DegToRadFactor)
					* Math.cos(pos2.lat * DegToRadFactor)
					* Math.cos((pos2.lng - pos1.lng) * DegToRadFactor))
					* R;
		};
		that.setup = function() {
		...
		that.getCanvasXY = function(pos) {
			var xy = that.canvasoverlay
					.fromLatLngToDivPixel(new google.maps.LatLng(pos.lat,
							pos.lng));
		...
		that.getDivXY = function(pos) {
			return that.canvasoverlay
					.fromLatLngToDivPixel(new google.maps.LatLng(pos.lat,
							pos.lng));
		};
		that.getLatLngFromWindowXY = function(x, y) {
			var latlng = that.canvasoverlay.getProjection()
		...
		that.getLatLngFromCanvasXY = function(x, y) {
			...
			var latlng = that.canvasoverlay.getProjection()
					.fromDivPixelToLatLng(new google.maps.Point(x, y), true);
			return {
				lat : latlng.lat(),
				lng : latlng.lng()
			};
	...
	func.forCoordinates = function(coords, callback) {
		$.getJSON(
				"http://www.mapnificent.net/api/checkCoordinates/?callback=?",
				...
				function(mapnificent) {
					...
					var options = {
						...
						defaultStartAtPosition : {
							lat : mapnificent.map.getCenter().lat(),
							lng : mapnificent.map.getCenter().lng()
						},
					...
					if (debugging) {
						options.workerURL = "/media/js/worker.js";
					}
					var localDefault = {};
					...
					options.reportInterval = Math
							.round(options.estimatedMaxCalculateCalls / 20);
					var LOCK = false, numberOfCalculations = 0, canCalculate = false, positionCounter = -1, stationList = [], blockGrid = undefined, stations = {}, lines = {}, colorCache = {}, blockCountX = undefined, blockCountY = undefined, blockSize = 0.5, imageData = null, coveredArea = false;
					...
					var getCrossDomainWorkerFacade = function() {
						var loaded = false, workers = {}, workerCounter = 0, iframeId = "mapnificentCrossDomain", targetOrigin = "http://www.mapnificent.net";
						...
						if (iframe === undefined) {
							...
							iframe.src = "http://www.mapnificent.net/media/api/"
									+ options.apiVersion + "/mapnificent.html";
					...
					var debug = {
						...
						error : function() {
							if (!!console) {
								console.error(Array.prototype.slice
										.call(arguments));
							}
						}
					};
					...
					var Position = (function() {
						var numberOfCalculations = 0, positions = {}, positionCounter = -1, startPositionsCount = 0, secondSorted = false, calculationsInProgress = 0;
						return {
							add : function(latlng, time) {
								positionCounter += 1;
								startPositionsCount += 1;
								var pos = {
									time : time === undefined ? options.maxWalkTime
											: time,
									latlng : latlng,
									running : false,
									ready : false,
									...
									calculate : function(doneclb, updateclb) {
										...
										var numberOfClosest = 3, minDistances = [], minStations = [], i = 0, j, nextStations = [], distances = [], indizes;
										try {
											while (i <= 1
													|| nextStations.length == 0) {
												indizes = getBlockIndizesForPositionByRadius(
														this["latlng"], i);
												for (j = 0; j < indizes.length; j += 1) {
													if (blockGrid[indizes[j][0]][indizes[j][1]].length > 0) {
														nextStations = Helper
																.merge(
																		nextStations,
																		blockGrid[indizes[j][0]][indizes[j][1]])
													}
												}
												i += 1;
									...
									draw : function(ctx, pos, stationId,
											pixelPerSecond, fullpath, prefunc) {
										if (stationId) {
										...
										var reachableIn = !!stationId ? this.stationMap[stationId]
												: 0;
										var secs = Math
												.min(
														(this.animatedSeconds - reachableIn),
														options.maxWalkTime);
										var radius = Math.max(secs
												* pixelPerSecond, 1);
										var nxy = mapnificent.getCanvasXY(pos);
										if (prefunc) {
											prefunc(ctx, pos, reachableIn,
													secs, nxy, radius);
										}
										ctx.moveTo(nxy.x, nxy.y);
										ctx.arc(nxy.x, nxy.y, radius, 0,
												mapnificent.env.circleRadians,
												true);
										if (!fullpath && !options.intersection) {
											ctx.fill();
										}
									...
									getGeoJson : function() {
										...
										var features = [];
										var secs = Math.min(
												this.animatedSeconds,
												options.maxWalkTime);
										var radius = secs
												* (1 / options.secondsPerKm)
												* 1000;
										features.push({
											type : "Feature",
											properties : {
										...
										w.document.body.innerHTML = JSON
												.stringify(geojson);
									}
								};
									...
									if (event.data.status == "done") {
										...
										if (event.data.status == "working") {
											...
											if (pos.updateCallback) {
												pos.updateCallback.call(pos,
														event.data.at);
											}
											Event.trigger("calculationUpdated",
													pos);
										}
									}
								};
							...
							draw : function(ctx, fullpath) {
								var count = 0;
								var pixPerSeconds = (1 / options.secondsPerKm)
										* options.pixelPerKm;
								for ( var index in positions) {
									...
									ctx.beginPath();
									pos.draw(ctx, null, null, pixPerSeconds,
											true);
									if (!fullpath && !options.intersection) {
										ctx.fill();
									}
							...
							drawColor : function(ctx) {
								...
								var pixPerSeconds = (1 / options.secondsPerKm)
										* options.pixelPerKm;
								var addSecondGradient = function(ctx, pos,
										seconds, secs, nxy, radius) {
									var grad = ctx.createRadialGradient(nxy.x,
											nxy.y, 0, nxy.x, nxy.y, radius);
									grad.addColorStop(0, getColorFor(seconds));
									grad.addColorStop(0.5, getColorFor(Math
											.floor(seconds + (secs / 2))));
									grad.addColorStop(1, getColorFor(seconds
											+ secs));
									ctx.fillStyle = grad;
								};
					...
					that.search = (function(that) {
						var UnionFind = function() {
							var num_weights = {}, parent_pointers = {}, num_to_objects = {}, objects_to_num = {}, object_num = 0;
							return {
									...
									for (var i = 0; i < w; i++) {
										...
										if (colorCheck(pix[current])) {
											if (colorCheck(pix[current - 4])
													&& colorCheck(pix[current
															- (w * 4)])) {
												if (uf.find(current - 4) === uf
														.find(current - (w * 4))) {
													uf.find(current);
													uf.union(current,
															current - 4);
					...
					that.inRange = function(pos) {
						if (pos.lat > options.northwest["lat"]
								|| pos.lat < options.southeast["lat"]
								|| pos.lng < options.northwest["lng"]
								|| pos.lng > options.southeast["lng"]) {
							return false;
						}
						return true;
					};
					...
					var getBlockIndizesForPosition = function(lat, lng) {
						var indexX = Math
								.floor((options.widthInKm
										/ options.latLngDiffs["lng"] * (lng - options.northwest["lng"]))
										/ blockSize);
						var indexY = Math
								.floor((options.heightInKm
										/ options.latLngDiffs["lat"] * (options.northwest["lat"] - lat))
										/ blockSize);
						return [ indexX, indexY ];
					};
					var getAlternativeBlockIndizesForPosition = function(lat,
							lng) {
						var indexX = Math.floor(mapnificent.getDistanceInKm(
								pos, {
									lat : lat,
									lng : options.northwest["lng"]
								})
								/ blockSize);
						var indexY = Math.floor(mapnificent.getDistanceInKm(
								pos, {
					...
					var getBlockIndizesForPositionByRadius = function(pos, rad,
							all) {
						var indizes = getBlockIndizesForPosition(pos.lat,
								pos.lng);
						if (rad === 0) {
							return [ indizes ];
						}
					...
					that.setup = function(controlcontainer, userOptions) {
						...
						options.latLngDiffs = {
							lat : Math.abs(options.northwest["lat"]
									- options.southeast["lat"]),
							lng : Math.abs(options.northwest["lng"]
									- options.southeast["lng"])
						};
						options.widthInKm = mapnificent.getDistanceInKm(
								options.northwest, options.northeast);
						options.heightInKm = mapnificent.getDistanceInKm(
								options.northwest, options.southwest);
					...
					that.calculatePixelPerKm = function() {
						options.southeastxy = mapnificent
								.getDivXY(options.southeast);
						options.northwestxy = mapnificent
								.getDivXY(options.northwest);
						options.southwestxy = mapnificent
								.getDivXY(options.southwest);
						options.northeastxy = mapnificent
								.getDivXY(options.northeast);
						options.map_width = Math.abs(options.southwestxy["x"]
								- options.northeastxy["x"]);
						options.map_height = Math.abs(options.southwestxy["y"]
								- options.northeastxy["y"]);
						options.pixelPerKm = options.map_width
								/ options.widthInKm;
					};
					that.numberOfPixelsToSqkm = function(numpix) {
						return Math
								.round(numpix
										* ((1 / options.pixelPerKm) * (1 / options.pixelPerKm))
										* 100) / 100;
					};
					...
					var loadDataPart = function(index) {
						loadDataScript(options.dataUrlPrefix + options.cityData
								+ "-" + index + ".json", function() {
							loadPartComplete(index);
						});
					};
					var loadPartComplete = function(index) {
						var data = mapnificent.getLayerData(that.idname,
								index - 1);
						if (!!data[2]) {
					...
					var dataLoaded = function(fromCache) {
						if (fromCache === undefined && !!localStorage) {
							try {
								localStorage.clear();
								localStorage.setItem(options.cityData, JSON
										.stringify([ stations, lines ]));
							} catch (e) {
							}
						}
						blockGrid = [];
						...
						for ( var stationId in stations) {
							...
							var indizes = getBlockIndizesForPosition(
					...
					var getColorFor = function(secs) {
						...
						if (colorCache[min] === undefined) {
							colorCache[min] = "hsla("
									+ (options.colorBaseGradientColor - Math
											.floor(min / options.colorMaxAcceptableTime
										    * (options.colorBaseGradientColor + options.colorMaxGradientColor)))
									+ ", 100%, 50%, 0.75)"
						}
					...
					var fillGreyArea = function(ctx) {
						...
						if (mapnificent.map.getMapTypeId() === "Night") {
							...
							if (options.dayTimeEnabled) {
								if (options.intervalKey[1] === "0"
										|| options.intervalKey[1] === "4") {
									options.darkOverlayColor = "rgba(0,0,0,0.8)"
								} else {
						...
						ctx.fillRect(0, 0, mapnificent.canvas.width,
								mapnificent.canvas.height)
					};
					var drawBounds = function(ctx, light) {
						...
						ctx.lineWidth = 1;
						ctx.beginPath();
						var nwxy = mapnificent.getCanvasXY(options.northwest), nexy = mapnificent
								.getCanvasXY(options.northeast), sexy = mapnificent
								.getCanvasXY(options.southeast), swxy = mapnificent
								.getCanvasXY(options.southwest);
...
//		}, 500);
	...
	that.bind("positionRemoved",
			function(index) {
				...
				$.address.deleteParameters([ "lat" + index, "lng" + index,
						"t" + index ]);
	...
	that.bind("calculationStarted", function(position) {
		...
		$("#" + idname + "-" + index + "-progressbar").find(
				".ui-progressbar-value").css("background", "");
	...
	that.bind("calculationUpdated", function(position) {
		...
		if (count > max + that.getOption("reportInterval")) {
			$("#" + idname + "-" + index + "-progressbar").find(
					".ui-progressbar-value").css("background",
					"url(/media/img/loading.gif) 40% center no-repeat");
		}
		$("#" + idname + "-" + index + "-progressbar").progressbar("value",
				percent);
	});
	google.maps.event.addListener(mapnificent.map, "dragend", function() {
		updateMapPositionAndZoom();
	});
	google.maps.event.addListener(mapnificent.map, "zoom_changed", function() {
		updateMapPositionAndZoom();
	});
	var updateMapPositionAndZoom = function(setnow, lat, lng, zoom) {
		if (setnow) {
			if (lat !== undefined && lng !== undefined) {
				mapnificent.map.setCenter(new google.maps.LatLng(
	...
	var updateCoveredArea = function() {
		...
		jQuery("#" + that.idname + "-coveredarea").text(
				numpix + " Pixel ~ " + that.numberOfPixelsToSqkm(numpix)
						+ " sqkm");
	};
	...
	var addPosition = function(latlng, time) {
			...
			infowindow : new google.maps.InfoWindow(
					{
						content : '<div style="height:110px;margin:8px 0px"><strong>Currently at:&nbsp;</strong><span class="'
								...
								+ "-"
								+ index
								+ '-address">Address Loading...</span><br/><strong>Or go to an address:</strong><form id="'
								+ idname
								+ "-"
								+ index
								+ '-addressinputform">'
		...
		google.maps.event
				.addListener(
						...
						"domready",
						function() {
							$("." + idname + "-" + index + "-address").text(
									startPositions[index].address);
							$("#" + idname + "-" + index + "-addressinputform")
									.submit(
											function(e) {
												e.preventDefault();
												getPointForAddress(
														...
														function(latlng) {
															if (that.inRange({
																...
																moveMarkerTo(
																		startPositions[position.index].marker,
																		position.latlng);
																showMessage("Your point is out of the covered area.");
																mapnificent
																		.trigger("redraw");
	...
	var updateGoby = function(e) {
		...
		if (newSecondsPerKm != that.getOption("secondsPerKm")
				|| newMaxWalkTime != that.getOption("maxWalkTime")) {
			that.setOption("secondsPerKm", newSecondsPerKm);
			that.setOption("maxWalkTime", newMaxWalkTime);
			var remove = [], add = {};
			if (that.getOption("secondsPerKm") === that
					.getOption("secondsPerKmWalking")) {
				remove.push("bike");
			} else {
				add.bike = "true";
			}
	...
	var dayTimeChanged = function() {
		...
		var ind = Math
				.floor($("#" + idname + "-daytimeslider").slider("value") / 100);
	...
	var setDayTime = function(dt) {
		...
		var dayTimeSliderPos = $("#" + idname + "-daytimeslider").slider(
				"value");
	...
	var appendControlHtmlTo = function() {
		...
		container
				.html('<div id="'
						+ idname
						+ '-positionContainer" class="positions"></div><div class="dataloading" id="'
						+ idname + '-loading"></div>');
		if ($.browser.webkit) {
			$("#clear-search").hide();
		}
		...
		if (mapnificent.env.hasCompositing) {
			...
			inter = '<label class="' + idname + '-intersection" for="nishishui">Intersect :&nbsp;</label><input'
					+ inter + ' class="' 
					+ idname
					+ '-intersection" type="checkbox" id="woshiintersection"/>';
		} else {
		...
		if (that.getOption("dayTimeEnabled")) {
			add = '<div class="daytime"><h4>Experimental: Set Time Of Day and Weekday</h4><div style="float:right; margin:0 10px"><input type="radio" class="'
					...
					+ idname + '-daytime-day" id="'
					+ idname + '-daytime-day-mon" value="mon" checked="checked"/><label for="'
					+ idname + '-daytime-day-mon">Mon-Fri</label><input type="radio" class="'
					+ idname + '-daytime-day" name="'
					+ idname + '-daytime-day" id="'
					+ idname + '-daytime-day-sat" value="sat"/><label for="'
					+ idname + '-daytime-day-sat">Saturday</label><input type="radio" class="'
					+ idname
					+ '-daytime-day" name="'
					+ idname
					+ '-daytime-day" id="'
					+ idname
					+ '-daytime-day-sun" value="sun"/><label for="'
					+ idname
					+ '-daytime-day-sun">Sunday</label></div><div><div id="'
					+ idname
					+ '-daytimeslider"></div><div style="width:50%;text-align:justify;font-size:6pt">12AM 3AM 6AM 9AM 12PM 3PM 6PM 9PM 12AM</div></div></div>';
		}
		container.after('<div class="contentoverlay" style="display:none" id="configure"><a class="close toggle" href="#configure">close</a><h2>Mapnificent Settings</h2><p><label for="'
						+ idname
						+ '-gobybike">Do you have a bike with you? </label><input type="checkbox" class="'
						+ idname
						+ '-goby" id="'
						+ idname
						+ '-gobybike" name="'
						+ idname
						+ '-goby" value="bike"/></p><p><label for="'
						+ idname
						+ '-gotime">Max. time to walk/ride from/to stations: </label><input size="4" type="text" id="'
						+ idname
						+ '-gotime" value="'
						+ Math.floor(that.getOption("maxWalkTime") / 60)
		...
		$('#intersect-container').append(inter);
		$('#woshicolor').change(function(){
		...
		if (!mapnificent.env.hasCompositing) {
			...
			$("." + idname + "-intersection").change(function() {
		...
		var newMarkerOptions = {
			stop : function(e) {
				...
				var mapOffset = $(mapnificent.map.getDiv()).offset();
	...
	var openPositionWindow = function(index) {
		return function() {
			startPositions[index].infowindow.open(mapnificent.map,
					startPositions[index].marker);
		};
	};
	...
	var addPositionHtml = function(index) {
		...
		$("#" + idname + "-positionContainer")
				.prepend(
						'<div id="' 
								+ idname + "-" + index + '" class="position-container"><span style="visibility:hidden" id="'
								+ idname + "-" + index + '-info">At most <strong id="'
								+ idname + "-" + index + '-timeSpan"></strong> minutes to any point in the highlighted area (estimate)</span> <small><a href="#" style="display:none" class="'
								+ idname + "-" + index + '-geojson">GeoJSON</a></small><div><input type="button" value="X" id="'
								+ idname + "-" + index + '-remove" class="remove-button"/><div style="display:none" id="'
								+ idname + "-" + index + '-slider"></div><div style="display:none" id="'
								+ idname + "-" + index + '-progressbar"></div></div><div style="font-size:9px;" class="'
								+ idname + "-" + index + '-address"></div></div>');
		...
		$("#" + idname + "-" + index + "-timeSpan").text(
				Math.round(startPositions[index].time / 60));
	...
	var highlightPositionArea = function(index) {
		...
		if (index !== undefined) {
			$("#" + idname + "-" + index).css("outline",
					"1px rgb(0,187,11) solid");
		}
	};
	...
	var displayPendingLoad = function(doit) {
		highlightPositionArea();
		if (doit) {
			$("#" + idname + "-loading").css("background",
					"url(/media/img/loading.gif) center center no-repeat")
					.progressbar("destroy").show()
		} else {
			$("#" + idname + "-loading").show().css("background", "")
					.progressbar({
						value : 0
					});
		}
	...
	var hashChange = function(event) {
		...
		for (index in startPositions) {
			...
			if (params["t" + index] !== undefined
				...
				setSecondsForIndex(parseInt(params["t" + index], 10) * 60,
						index);
			}
		}
		for (index in recalc) {
		...
		for ( var key in params) {
			for (var i = 0; i < searchFor.length; i++) {
				if (key.indexOf(searchFor[i]) === 0) {
					index = parseInt(key.substring(searchFor[i].length,
							key.length), 10);
	...
	var addMarker = function(jobResult) {
	    ...
	    google.maps.event.addListener(jobResult.marker, 'mouseover', function(){
	    	jobResult.infowindow.open(mapnificent.map,jobResult.marker);
	    });
	...
	var searchArea = function() {
		...
		for (var i = 0; i < blobs.length; i++) {
			...
			(function() {
				...
				var pyrmont = mapnificent.getLatLngFromCanvasXY(blobs[i].midx, blobs[i].midy);
				var request = {
					    location: pyrmont,
					    radius: '500',
					    query: query
				...
				var service = new google.maps.places.PlacesService(mapnificent.map);
...
//				bsearcher.setSearchCompleteCallback(bsearcher, function() {
...
//				bsearcher.execute(query);
	...
	/*Visualize the search result on the map*/
